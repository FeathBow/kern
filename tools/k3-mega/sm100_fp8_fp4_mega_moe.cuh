// kern's fork of DeepGEMM `impls/sm100_fp8_fp4_mega_moe.cuh` (pegainfer's
// vendored copy with the K3 situ activation, upstream sha256 e74a82f9c931ddf7).
...
sm100_fp8_fp4_mega_moe_body(void* y,
                            ...
                            const cute::TmaDescriptor& tensor_map_shared_l1_weights_sf,
                            ...
                            const cute::TmaDescriptor& tensor_map_shared_l2_weights_sf) {
            ...
            if (task_info.block_phase == sched::BlockPhase::Linear1 or task_info.block_phase == sched::BlockPhase::SharedLinear1) {
                ...
                for (uint32_t s = 0; s < WG_BLOCK_M / STORE_BLOCK_M; ++ s) {
                    ...
                    for (uint32_t i = 0; i < kNumAtomsPerStore; ++ i) {
                        ...
                        // Apply activation: SwiGLU `silu(gate) * up`, or Situ (Kimi K3)
                        ...
                        for (uint32_t k = 0; k < 2; ++ k) {
                            ...
                            float2 act;
                            if constexpr (kActivationSitu) {
                                // Situ: `4 * tanh(gate / 4) * sigmoid(gate) * 25 * tanh(up / 25)`
                                const auto sig = kFastMath ?
                                    make_float2(math::fast_rcp(denom.x), math::fast_rcp(denom.y)) :
                                    make_float2(1.0f / denom.x, 1.0f / denom.y);
                                act.x = 4.0f * tanhf(gate.x / 4.0f) * sig.x * (25.0f * tanhf(up.x / 25.0f));
                                act.y = 4.0f * tanhf(gate.y / 4.0f) * sig.y * (25.0f * tanhf(up.y / 25.0f));
                            } else {
                                // SwiGLU
                                ...
                                act = __fmul2_rn(gate, up);
                            }
                            activation_values[i][k] = __fmul2_rn(act, weights);
                        }
