...
| 工具 | 输入 → 输出 |
|------|-------------|
| `kernels-src/k3_*.cu` + `build_kernels.sh` | **现行 K3 decode 核集**（2026-09-02，CUDA C++ 重写，契约 ../docs/k3-kernel-abi.md）：`k3_residual`（attnres_rms / land_add_attnres_rms / land_add2）、`k3_conv_silu`、`k3_kda_core`、`k3_mla_prep`、`k3_mla_paged_attn`（cluster-8 split-KV，32k ctx 258 µs）、`k3_router_argmax`（router_topk / argmax / rms）、`k3_land`；elementwise（add、mul_sigmoid、norm、cuBLAS f32 partial 的 landing）都融进邻居核，每 rank B 是变量（≤ 64）。`k3_kv_append.cu`、`k3_mega_stage.cu`：latent 追加、MegaMoE 输入 staging |
...
| `k3-tilelang/` + `build_k3_kernels.sh` + `k3_line_shim.py` | **旧核集**（E2 时期，pegainfer 的 AOT TileLang 核 + line 寻址 shim，每 rank B=1）；`examples/k3-*.json`（无 `-v2`）仍用它，留到消融时决定去留 |
...
| `gen_k3_decode.py` | `--layers N --ranks R --max-ctx C --seqs S` → `examples/k3-<N>l-ep<R>-v2.json` / `k3-ep<R>-v2.json`：整条 decode program（93 层 1855 launch，742 个 cuBLAS GEMM），`tokens`/`seqs` 是变量，几何在 `GEOM` 表里；MoE 三步来自 `gen_k3_moe.mega_pieces`。核改了要先 `KERN_REBUILD=1 tools/build_kernels.sh` 再生成（manifest 钉 cubin 哈希） |
