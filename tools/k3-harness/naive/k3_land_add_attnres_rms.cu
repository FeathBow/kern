...
// [K1b] naive reference transcription — one block per row, 1024 threads.
//   grid (B,1,1)  block (1024,1,1)  dynamic smem 0
// p = bf16(partial); prefix2 = snapshot ? p : bf16(prefix + p);
...
extern "C" __global__ void kern_k3_land_add_attnres_rms(
    const float* __restrict__ partial, const bf16* __restrict__ prefix,
    const bf16* __restrict__ blocks, const float* __restrict__ sw,
    const bf16* __restrict__ gamma, bf16* __restrict__ prefix2,
  ...
  __shared__ bf16 p2[K3_H];
  __shared__ bf16 mixed[K3_H];

  const bf16* blk = blocks + (size_t)b * K3_NB_MAX * K3_H;
  for (int i = tid; i < K3_H; i += nt) {
    bf16 p = f2b(partial[(size_t)b * K3_H + i]);
    bf16 v = snapshot ? p : f2b(b2f(prefix[(size_t)b * K3_H + i]) + b2f(p));
    p2[i] = v;
    prefix2[(size_t)b * K3_H + i] = v;
  }
  __syncthreads();
  ...
  if (nb > 0) {
    for (int c = 0; c < nc; ++c) {
      const bf16* cand = (c < nb) ? (blk + (size_t)c * K3_H) : p2;
      float ss = 0.0f;
