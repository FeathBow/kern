...
//! `schema/manifest-v2.schema.json` is golden-checked in CI:
//!
//! ```sh
//! cargo run -p kern-manifest --example gen_schema > schema/manifest-v2.schema.json
//! ```
...
fn main() {
    ...
    v["$id"] =
        "https://kern-baa.pages.dev/schema/manifest-v2.schema.json".into();
    v["title"] = "kern manifest v2".into();
